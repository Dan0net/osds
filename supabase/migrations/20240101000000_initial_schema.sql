...
create policy "Anyone can read walker users" on public.users
  for select using (
    exists (
      select 1 from public.walker_profiles wp where wp.user_id = users.id
    )
  );
...
-- ical_cache: service role only — deny all client access
create policy "No client access" on public.ical_cache
  for all using (false);
