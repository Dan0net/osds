...
import { apiFetch, stripeConnectOnboard, stripeConnectCallback, stripeDashboardLink } from '../../lib/api'
import { clientPriceCents } from '../../lib/utils'

const DAYS = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']

export default function AccountWalkerPage() {
  const { user, walkerProfile, refreshProfile } = useAuth()

  // Profile fields
  const [form, setForm] = useState({
    business_name: '', slug: '', bio: '', theme_color: '#4f46e5', city: '', postcode: '',
  })
  ...
  const [availability, setAvailability] = useState(
    DAYS.map((day, i) => ({ day, day_of_week: i + 1, enabled: false, start_time: '09:00', end_time: '17:00' })),
  )
  ...
  useEffect(() => {
    if (walkerProfile?.stripe_account_id) {
      stripeConnectCallback().then((res) => { if (res.data) setStripeStatus(res.data) })
    }
  ...
  useEffect(() => {
    ...
    if (!walkerProfile.calendar_feed_token) {
      ...
      supabase.from('walker_profiles').update({ calendar_feed_token: token }).eq('id', walkerProfile.id)
    }
  }, [walkerProfile?.id])
  ...
  async function loadServices() {
    const { data } = await supabase.from('services').select('*').eq('walker_id', walkerProfile.id).order('created_at')
    setServices(data || [])
  }
  function startAddSvc() { setEditingSvc('new'); setSvcForm({ name: '', price_cents: '', duration_minutes: '', service_type: 'standard' }) }
  ...
  async function saveSvc() {
    setSvcLoading(true)
    const data = { name: svcForm.name, price_cents: Math.round(parseFloat(svcForm.price_cents) * 100), duration_minutes: parseInt(svcForm.duration_minutes), service_type: svcForm.service_type }
    if (editingSvc === 'new') await supabase.from('services').insert({ ...data, walker_id: walkerProfile.id })
    else await supabase.from('services').update(data).eq('id', editingSvc)
    setEditingSvc(null); setSvcLoading(false); await loadServices()
  }
  async function toggleActive(id) {
    const svc = services.find((s) => s.id === id); if (!svc) return
    await supabase.from('services').update({ active: !svc.active }).eq('id', id); await loadServices()
  }

  // --- Availability ---
  async function loadAvailability() {
    const { data } = await supabase.from('availability').select('*').eq('walker_id', walkerProfile.id)
    setAvailability(DAYS.map((day, i) => {
      const existing = (data || []).find((a) => a.day_of_week === i + 1)
      return { id: existing?.id, day, day_of_week: i + 1, enabled: !!existing, start_time: existing?.start_time?.slice(0, 5) || '09:00', end_time: existing?.end_time?.slice(0, 5) || '17:00' }
    }))
  }
  function toggleDay(dayOfWeek) { setAvailability((prev) => prev.map((a) => a.day_of_week === dayOfWeek ? { ...a, enabled: !a.enabled } : a)) }
  function updateTime(dayOfWeek, field, value) { setAvailability((prev) => prev.map((a) => a.day_of_week === dayOfWeek ? { ...a, [field]: value } : a)) }
  async function saveAvailability() {
    setAvailSaving(true)
    await supabase.from('availability').delete().eq('walker_id', walkerProfile.id)
    const enabled = availability.filter((a) => a.enabled)
    if (enabled.length > 0) {
      await supabase.from('availability').insert(enabled.map((a) => ({ walker_id: walkerProfile.id, day_of_week: a.day_of_week, start_time: a.start_time, end_time: a.end_time })))
    }
    setAvailSaving(false); await loadAvailability()
  }

  // --- Blocked dates ---
  async function loadBlockedDates() {
    const { data } = await supabase.from('blocked_dates').select('*').eq('walker_id', walkerProfile.id).order('date')
    setBlockedDates(data || [])
  }
  async function addBlockedDate() {
    if (!newBlock.date) return
    await supabase.from('blocked_dates').insert({ walker_id: walkerProfile.id, date: newBlock.date, reason: newBlock.reason })
    setNewBlock({ date: '', reason: '' }); await loadBlockedDates()
  }
  async function removeBlockedDate(id) { await supabase.from('blocked_dates').delete().eq('id', id); await loadBlockedDates() }

  // --- Calendar imports ---
  async function loadIcalImports() {
    const { data } = await supabase.from('ical_imports').select('*').eq('walker_id', walkerProfile.id).order('created_at')
    setIcalImports(data || [])
  }
  async function addIcalImport() {
    setImportError(null)
    if (!importForm.label.trim() || !importForm.url.trim()) { setImportError('Label and URL are required'); return }
    if (!importForm.url.startsWith('https://')) { setImportError('URL must start with https://'); return }
    setImportValidating(true)
    try {
      const res = await apiFetch('validate-ical-url', { method: 'POST', body: JSON.stringify({ url: importForm.url.trim() }) })
      if (!res.data?.valid) { setImportError(res.data?.error || 'Could not validate URL'); return }
    } catch { setImportError('Failed to validate URL'); return }
    finally { setImportValidating(false) }
    const { error } = await supabase.from('ical_imports').insert({ walker_id: walkerProfile.id, label: importForm.label.trim(), url: importForm.url.trim() })
    if (error) { setImportError(error.message); return }
    setImportForm({ label: '', url: '' }); await loadIcalImports()
  }
  async function removeIcalImport(id) { await supabase.from('ical_imports').delete().eq('id', id); await loadIcalImports() }
  function handleCopy() { navigator.clipboard.writeText(feedUrl); setCopied(true); setTimeout(() => setCopied(false), 2000) }
  async function handleRegenerateToken() {
    ...
    await supabase.from('walker_profiles').update({ calendar_feed_token: crypto.randomUUID() }).eq('id', walkerProfile.id)
  ...
  if (!walkerProfile) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500">You don't have a walker profile yet.</p>
      </div>
    )
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Walker Page</h1>
        <a href={`/w/${form.slug}`} target="_blank" rel="noopener noreferrer" className="text-sm text-indigo-600 hover:underline">
          Preview page →
        </a>
      </div>
      ...
      <form onSubmit={handleSaveProfile} className="mb-8">
          ...
          <div>
            ...
            <div className="flex items-center gap-1">
              ...
              <input type="text" value={form.slug} onChange={(e) => update('slug', e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ''))}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none" placeholder="your-slug" />
            </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={form.city} onChange={(e) => update('city', e.target.value)}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none" placeholder="e.g. Edinburgh" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Postcode</label>
              <input type="text" value={form.postcode} onChange={(e) => update('postcode', e.target.value)}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none" placeholder="e.g. EH1 1AA" />
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            {stripeStatus?.charges_enabled ? (
              <div className="space-y-2">
                ...
                <button type="button" onClick={async () => { const res = await stripeDashboardLink(); if (res.data?.url) window.open(res.data.url, '_blank') }}
                  className="border border-gray-300 text-gray-700 text-sm font-medium px-4 py-2 rounded-lg hover:bg-gray-50">Open Stripe Dashboard</button>
              </div>
              ...
              <div className="space-y-2">
                ...
                <button type="button" disabled={stripeLoading} onClick={async () => {
                  setStripeLoading(true); const res = await stripeConnectOnboard(); if (res.data?.url) window.location.href = res.data.url; else setStripeLoading(false)
              ...
              <div>
                <button type="button" disabled={stripeLoading} onClick={async () => {
                  setStripeLoading(true); const res = await stripeConnectOnboard()
                  if (res.data?.url) window.location.href = res.data.url
                  else { setError(res.error || 'Failed to start Stripe onboarding'); setStripeLoading(false) }
                }} className="bg-indigo-600 text-white text-sm font-medium px-4 py-2 rounded-lg hover:bg-indigo-700 disabled:opacity-50">
        ...
        {error && <div className="bg-red-50 border border-red-200 text-red-700 text-sm rounded-lg px-4 py-3 mt-4">{error}</div>}
        <div className="flex items-center gap-3 mt-4">
          <button type="submit" disabled={saving} className="bg-indigo-600 text-white font-semibold px-6 py-2.5 rounded-lg hover:bg-indigo-700 disabled:opacity-50">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={startAddSvc} className="bg-indigo-600 text-white text-sm font-medium px-4 py-2 rounded-lg hover:bg-indigo-700">Add service</button>
        </div>
        {editingSvc && (
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              ...
              <div>
                ...
                {svcForm.price_cents && parseFloat(svcForm.price_cents) > 0 && (
                  <p className="text-xs text-gray-500 mt-1">Client pays £{(clientPriceCents(Math.round(parseFloat(svcForm.price_cents) * 100)) / 100).toFixed(2)}{svcForm.service_type === 'overnight' ? '/night' : ''}</p>
                )}
              </div>
              <div>
          ...
          {services.map((svc) => (
            <div key={svc.id} className={`bg-white border border-gray-200 rounded-lg p-4 flex items-center justify-between ${!svc.active ? 'opacity-50' : ''}`}>
              <div>
                ...
                {svc.service_type === 'overnight' && <span className="ml-2 text-xs font-medium bg-indigo-100 text-indigo-700 px-1.5 py-0.5 rounded">overnight</span>}
          ...
          <div className="space-y-2">
            {blockedDates.map((block) => (
              <div key={block.id} className="flex items-center justify-between bg-gray-50 rounded-lg px-3 py-2">
                <div>
                  <span className="font-medium text-sm">{new Date(block.date).toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' })}</span>
      ...
      <div className="mb-8">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4 mb-4">
          ...
          <p className="text-sm text-gray-500 mb-3">Add iCal feed URLs to block your availability.</p>
          ...
          {icalImports.length === 0 && <p className="text-sm text-gray-400 mb-4">No calendars added yet.</p>}
          <div className="flex flex-col sm:flex-row gap-2">
            <input type="text" value={importForm.label} onChange={(e) => { setImportForm({ ...importForm, label: e.target.value }); setImportError(null) }}
              placeholder="Label (e.g. Personal, Rover)" className="border border-gray-300 rounded-lg px-3 py-2 text-sm sm:w-40 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none" />
            <input type="url" value={importForm.url} onChange={(e) => { setImportForm({ ...importForm, url: e.target.value }); setImportError(null) }}
              placeholder="https://calendar.google.com/calendar/ical/..." className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none" />
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleCopy} className="border border-gray-300 text-gray-700 text-sm font-medium px-4 py-2 rounded-lg hover:bg-gray-50">{copied ? 'Copied!' : 'Copy'}</button>
          </div>
          <button onClick={handleRegenerateToken} className="text-sm text-red-500 hover:text-red-700 mt-2">Regenerate URL</button>
        </div>
      </div>
    </div>
  )
